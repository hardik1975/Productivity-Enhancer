...
#define WIFI_SSID "yo1"
#define WIFI_PASSWORD "hardikhardik"

#define DATABASE_URL "https://test-21sep-7648d-default-rtdb.asia-southeast1.firebasedatabase.app"
...
void setup()
{
    ...
    String name1 = Database.push<bool>(client, "/test/push", true);
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name1: %s\n", name1.c_str());
    ...
    String name2 = Database.push<String>(client, "/test/push", "hello");
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name2: %s\n", name2.c_str());
    ...
    String name3 = Database.push<number_t>(client, "/test/push", number_t(123.456, 2));
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name3: %s\n", name3.c_str());
    ...
    String name4 = Database.push<number_t>(client, "/test/push", number_t(1234.56789, 4));
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name4: %s\n", name4.c_str());
    ...
    String name5 = Database.push<object_t>(client, "/test/push", object_t("{\"test\":{\"data\":123}}"));
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name5: %s\n", name5.c_str());
    ...
    String name6 = Database.push<object_t>(client, "/test/push", object_t("[1,2,\"test\",true]"));
    if (client.lastError().code() == 0)
        Firebase.printf("ok, name6: %s\n", name6.c_str());
